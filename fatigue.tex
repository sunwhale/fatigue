...
\begin{figure}[t]
\centering
\includegraphics[width=8.5cm]{figures/microstructure_200X.png}
\caption{Initial microstructure of the specimen section. \marked{The picture is not good. Use a higher amplification factor to show grain boundaries more clearly.}}
...
To gain insight into the failure mechanism of multiaxial thermomechanical fatigue, scanning electron microscope (SEM) was used to investigate the fracture surfaces. In Figure \ref{Fig:crack_initiation}, typical fractographs in the region of crack initiation are shown. For both isothermal and thermomechanical loadings, crack initiation and subsequent failure have been identified as being initiated at the outer surface of the specimen, as indicated by arrows in Figure \ref{Fig:crack_initiation}. \marked{The SEM investigations reveal that the dominant failure mechanism is changing with the phase angle of the thermal loading and mechanical loading, $\theta_{T-\varepsilon}$, as well as the mechanical loading path. ({\em I did not find changes in the pictures. You have to make it more clear what is what. })}
...
\begin{figure*}
   \centering
   \begin{overpic}[width=8.0cm]{figures/7112-1.jpg}
   ...
   \end{overpic}
   \begin{overpic}[width=8.0cm]{figures/7036-1.jpg}
     \put(0,65){\fcolorbox{white}{white}{(f)}}
     \put(50,40){\color{white}\thicklines\vector(-1,0){30}}
   \end{overpic}
  \caption{Locations of crack initiation from thermomechanical fatigue tests. (a) TC-IF 0.45\%, (b) TC-IP 0.6\%, (c) TC-OP 0.65\%, (d) RPO-IP 0.6\%, (e) NPR-IP 0.7\%, (f) NPR-IP 0.5\%. Arrows denote the crack initiation locations. \marked{Six pictures can be reduced to two or four pictures. You should make your choice. One cannot see many different mechanisms. The locations are not important if they are not coupled with certain phenomena.}}
...
The fatigue endurance is determined from \marked{(Why the tension failure mode is assumed here for the shear model? Please check!)}
\begin{equation}
...
Zamrik and Renauld \cite{Zamrik2000} suggested an explicit fatigue model, as
\begin{equation}
\begin{aligned}
{N_{\rm f}} = A{\left[ {\left( {\frac{{{\varepsilon _{eq,ten}}}}{{{\varepsilon _{\rm f}}}}} \right)\left( {\frac{{{\sigma _{eq,\max }}}}{{{\sigma _{ult}}}}} \right)} \right]^B} \cdot \\
{\left( {1 + \frac{{{t_h}}}{{{t_c}}}} \right)^C}\exp \left( {\frac{{ - Q}}{{R\left( {{T_{\max }} - {T_0}} \right)}}} \right),
\end{aligned}
\label{Equ:StudyModel}
\end{equation}
where $\varepsilon _{eq,ten}$ denotes XXX and $\sigma _{eq,\max }$ is XXX. \marked{(You have to add comments and introductions to the model.}
...
\begin{figure*}
  ...
  \nonumber
    ...
    \begin{overpic}[width=7.5cm]{figures/NF-NP-TMF-Zamrik.pdf}
      \put(65,20){\fcolorbox{white}{white}{(a) Zamrik}}
    \end{overpic}
  \end{minipage}%
  \begin{minipage}[t]{0.5\linewidth}
    \centering
  ...
  \caption{Comparison between predicted fatigue life and experimental results of multiaxial thermomechanical fatigue tests. (a) Brown-Miller Model; (b) Fatemi-Socie Model; (c) Smith-Watson-Topper Model; (d) Chu-Conle-Bonnen Model; (e) Liu Tension Energy Model.}
...
\marked{(Add more details  about the life calculations and discussions about results here!)}
...
As given in \cite{Warren2008}, the activation energy $Q_0$ is set to the value of 240kJ/mol and the intrinsic activation volume $\upsilon _0^*$ is used the value of $3.51\times10^{-4}\rm{m}^{3}/\rm{mol}$. The ultimate stress $\sigma_{ult}$ of Inconel 718 at 650 $^\circ$C is determined from the experiment as $1305$MPa. The constants $k$, $B$ and $C$ are determined by the optimization method to minimize the life prediction mean-squared error. Finally, $k$ is set to 1.2, $B$ is set to 0.5 and $C$ is set to $1.29\times10^{12}s^{-1}$.
